...
. Ensure `hub.lab.example.com` node is in `Ready` status and all operators are available.
+
.Sample output
----
...
=== Create the AgentServiceConfig custom resource for connected environment
...
. Create the `AgentServiceConfig` custom resource.
+
[source,bash,role=execute]
----
cat >agent_service_config.yaml<<-"EOF"
...
+
[NOTE]
https://github.com/openshift/assisted-service/blob/master/data/default_os_images.json[Default_os_images]
